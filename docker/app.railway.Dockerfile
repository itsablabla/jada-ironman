...
# Same as app.Dockerfile but without BuildKit cache mounts
# (Railway does not support --mount=type=cache)
# ========================================
...
RUN HUSKY=0 bun install sharp --linker=hoisted
...
RUN python3 -m venv ./apps/sim/lib/guardrails/venv && \
